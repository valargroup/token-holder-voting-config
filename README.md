...
| [`dynamic-voting-config.json`](https://voting.valargroup.org/dynamic-voting-config.json) / [`static-voting-config.json`](https://voting.valargroup.org/static-voting-config.json) | Production default aliases | Generated by the Pages deploy from `prod/` so clients can omit the environment path and still get production. Do not edit root v2 files in source. |
| [`prod/dynamic-voting-config.json`](https://voting.valargroup.org/prod/dynamic-voting-config.json) | Production dynamic config, per-round signed registry | Active. Production wallet releases consume this through their hash-pinned production static config. |
| [`prod/static-voting-config.json`](https://voting.valargroup.org/prod/static-voting-config.json) | Production static config | Active. Production wallet releases hash-pin this file's bytes; any change requires a coordinated wallet release. See [Hash-pinning and wallet releases](#hash-pinning-and-wallet-releases). |
...
The root v2 URLs on `voting.valargroup.org` are deploy-generated aliases for
production. They are not checked-in source files.
...
which verifies it against `prod/dynamic-voting-config.json` and publishes
...
`static-voting-config.json.sha256` files beside the default production alias,
production config, and staging config for human verification. Wallets must trust the hash embedded
